...
エンジンのディレクトリ (`engines/`) も除外する。ライセンス表示から開くライセンス全文は
同一オリジンの URL を `window.open()` したもので、**ナビゲーションとして届く**ためである。
除外しないと、事前キャッシュしてあってもアプリ本体が返り、全文の代わりに
ShogiHome がもう 1 つ開く。除外した後は事前キャッシュのルートが `text/plain` のまま返す。
