...
起動のたびにマニフェストから組み立て直される。利用者がプリセットをそのまま編集しても
意図した設定が残るとは限らないため、**オプションを開く操作はコピーの作成に読み替える。**

エンジン管理ダイアログとプレイヤー選択 (`PlayerSelector`) の「設定」は、対象が
プリセットの URI (`uri.isBuiltinUSIEngine()`) の場合に確認ダイアログを出し、
了承されたらコピーを作ってそちらの設定を開く。プレイヤー選択では併せて選択も
コピーへ切り替える。コピーは `issueEngineURI()` の URI を持つ通常のエンジンなので、
localStorage にそのまま保存され、次回以降も編集した内容が残る。
