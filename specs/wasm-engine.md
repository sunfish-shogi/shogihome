...
`web.ts` の `loadUSIEngines()` は localStorage に保存された一覧へ組み込みエンジンを
足して返す。**保存されている内容は見ずにマニフェストの通りに上書きする。**
...
組み込みエンジンの一覧と各プリセットのオプション値はアプリ側が管理していて、
起動のたびにマニフェストから組み立て直される。上書きされるので、プリセットを直接
編集しても保存されず、一覧から消しても次の読み込みで戻ってくる。無効な操作を
させないために、**UI の側でプリセットを編集・削除させない。**

- **オプションを開く操作はコピーの作成に読み替える。** エンジン管理ダイアログと
  プレイヤー選択 (`PlayerSelector`) の「設定」は、対象がプリセットの URI
  (`uri.isBuiltinUSIEngine()`) の場合に確認ダイアログを出し、了承されたらコピーを
  作ってそちらの設定を開く。プレイヤー選択では併せて選択もコピーへ切り替える。
  コピーは `issueEngineURI()` の URI を持つ通常のエンジンなので、localStorage に
  そのまま保存され、次回以降も編集した内容が残る。
- **削除ボタンは出さない。** エンジン管理ダイアログの「削除」はプリセットには表示しない。

判定は URI だけで行う。この URI は Web 版の組み込みエンジンにしか割り当てられず、
Electron 版のエンジンは `issueEngineURI()` の URI を持つため、
**デスクトップ版の挙動は変わらない。**
