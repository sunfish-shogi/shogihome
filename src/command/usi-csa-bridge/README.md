...
連続対局 (`repeat` が 2 以上) では対局ごとに接続しなおされます。 `csa.log` では対局の区切りに `disconnect:` を記述し、切断を待ってから次の対局のやりとりを続けます。 `restartPlayerEveryGame` が有効な場合は対局ごとに USI エンジンのプロセスが起動しなおされるため、 `usi.log` には 1 局分のみを記述します (プロセスごとに先頭から再生されます)。 棋譜も対局ごとに保存されるため、`verify-record` では複数の棋譜を比較します。
