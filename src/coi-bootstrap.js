...
(function () {
  ...
  // このドキュメントを制御している Service Worker がいるのに isolated でない場合、
  // その Service Worker はヘッダーを付けない古い版である (新しい版が制御していれば、
  // このドキュメントに既にヘッダーが付いている)。再読み込みしても応答するのは
  // 同じ版なので isolated にはならず、下の試行の記録を無駄に消費してしまう。
  //
  // 新しい版は待機中のまま、src/renderer/webapp/update.ts の通知から適用されたときに
  // 制御を引き継ぐ。そのときの再読み込みで isolated になるので、ここでは登録だけ行って
  // (更新の確認を促して) そのまま起動する。
  //
  // ヘッダーを付けないページ (prompt / monitor / layout-manager) もここに該当する。
  // それらは Service Worker のナビゲーションのルートから除外してあり、
  ...
  navigator.serviceWorker
    ...
    .then(function () {
      ...
      //
      // ここへ来るのは制御中の Service Worker がいない場合だけなので、通常は
      // 今登録した版が最初の版として activate する。ただしハードリロードのように
      // Service Worker を迂回したナビゲーションでは、古い版が有効なままでも
      // controller が空になる。その場合は 1 度だけ試して記録し、次からは諦める。
