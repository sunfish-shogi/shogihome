...
function formatUSIEngineStatsAsHTMLTable(stats: USIEngineStatsEntry): string {
  ...
  if (stats.npsSampleCount > 0) {
    html += formatRow(
      "平均 NPS",
      `${stats.meanNPS.toFixed(0)} (最大 ${stats.maxNPS} サンプル数: ${stats.npsSampleCount})`,
    );
  }
  if (stats.maxDepth > 0) {
    html += formatRow("最大深さ", stats.maxDepth);
  }
  if (stats.maxSelDepth > 0) {
    html += formatRow("最大選択的深さ", stats.maxSelDepth);
  }
  if (stats.depth10Count > 0) {
    const mean = stats.depth10TotalTimeMs / stats.depth10Count / 1e3;
    html += formatRow("深さ 10 到達", `${stats.depth10Count} 回 / 平均 ${mean.toFixed(2)} 秒`);
  }
  ...
  if (stats.hashUsageSampleCount > 0) {
    html += formatRow(
      "平均ハッシュ使用率",
      `${stats.meanHashUsagePercent}% (最大 ${stats.maxHashUsagePercent.toFixed(2)}%)`,
    );
